...
def bad_index(flux,time):
    ...
    clip = input(prompt)
    index = np.asarray(np.where(flux<clip)[0])
    return index
...
def plot_flux(flux,flux_std,time):
    plt.figure()
    plt.errorbar(time,flux,yerr=flux_std,xerr=None,fmt=',',color='k')
    plt.show()
...
def bin_data(flux,flux_std,time,d_time,form):
    bin_time = []
    bin_data = []
    bin_err = []
    if isinstance(form,str):
        if form = 'Jul':
            d_time = d_time.split()
            d_time = Gre2Jul(int(d_time[0]),int(d_time[1]),int(d_time[2]),int(d_time[3]),int(d_time[4]),float(d_time[5]))
        elif form = 'Sec':
            d_time = d_time.split()
            d_time = Time2Sec(int(d_time[0]),int(d_time[1]),int(d_time[2]))
    set_time = time[-1]-d_time
    ...
    for i in reversed(range(len(A))):
        if time[i]>=set_time:
            ...
            bin_in_er.append(flux[i])
        else:
            aver_da = np.mean(np.asarray(bin_in_da))
            aver_er = np.mean(np.asarray(bin_in_er))
            bin_in_da = []
            bin_in_er = []
            bin_in_da.append(flux[i])
            bin_in_er.append(flux[i])
            set_time = set_time-d_time
    ...
    bin_data.append(aver_da)
    bin_err.append(aver_er)
    return np.array(bin_data),np.array(bin_err),np.array(bin_time)

def save_xls(path,file_list,ra,dec,n):
    wb = Workbook()
    ...
    wb.save('table of flux and error.xls')
